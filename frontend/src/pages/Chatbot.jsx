    ...
    <div className="flex flex-col md:flex-row h-screen bg-gradient-to-t from-blue-100 to-blue-200">
      ...
      <aside className="w-full md:w-1/4 bg-white shadow-lg border-r overflow-y-auto p-4">
        ...
        <ul className="space-y-2 ">
          {pdfList.map((pdf, index) => (
            <li
              key={index}
              className={`p-2 cursor-pointer rounded-lg transition mt-3 shadow-md  ${selectedPdf === pdf
        ...
        <button
          ...
          className="mt-4 bg-gray-500 self-end text-white py-2 px-4 rounded-lg hover:bg-gray-600 transition"
        >
          Quay lại Home
        </button>
      </aside>
      ...
      <main className="flex-grow flex flex-col items-center justify-center p-6">
        <div className="w-full max-w-full p-6 bg-white rounded-lg shadow-lg border border-gray-200">
          <div className="h-[31rem] overflow-y-auto mb-4 p-4 space-y-4">
            {messages.map((message, index) => (
              <div
                key={index}
                className={`p-4 rounded-lg max-w-[80%] break-words ${message.sender === "user"
              ...
              >
                <ReactMarkdown >
                  {message && message.text ?
                    message.text :
                    `I am a chatbot designed to answer questions about the PDF you have provided. 
